      ...
      <div className="bento-card overflow-hidden">
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800">
                ...
                <th className="p-4 text-xs font-semibold text-zinc-500 uppercase tracking-wider">Amount & Payment</th>
              ...
              {orders.length > 0 ? (
                orders.map((order) => (
                  <tr key={order.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/20 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="font-bold text-zinc-900 dark:text-white">${order.amount.toFixed(2)}</div>
                      <div className={`text-xs uppercase font-bold tracking-wider ${getPaymentBadgeClass(order.gateway)}`}>
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 flex justify-between items-center text-sm text-zinc-500">
          ...
          <div className="flex gap-2">
            ...
            {pages > 1 && (
              <button className="px-3 py-1 border border-zinc-200 dark:border-zinc-800 rounded">Next</button>
            )}
          </div>
        </div>
      </div>
